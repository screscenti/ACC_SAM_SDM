...
static inline char *  Unittest_Helper_bool_to_string(bool b);
...
void  Unittest_Helper_log_assert_fail_range_int(char const  *testName, char const  *expected, int64_t actual, char const  *nodeId, char const  *modelId, char const  *msg) 
{
  char buffer[255] = {
    0  };
  snprintf(buffer, 255, msg, actual, expected);
...
int32_t  Unittest_Helper_finish_testsuite(Unittest_Helper_TestSuite_t *(suites[]), uint32_t suiteCount) 
{
  ...
  for ( uint32_t i = 0 ; i < suiteCount; i++ )
  {
    Unittest_Helper_TestSuite_t *suite = suites[i];
    ...
    for ( uint8_t __c = 0 ; __c < suite->test_count; __c++ )
    {
      Unittest_Helper_Test_t *__it = &(suite->tests[__c]);
      ...
      FILE *fp_stdout = fopen(f_stdout, "r");
      if ( fp_stdout != NULL ) 
      {
        char buf[254] = {
          0        };
