...
void  ACC_SafeDistance_Atomic____ssd_safe_distance_computation_execute(ACC_SafeDistance_Atomic____ssd_safe_distance_computation_data_t *___id, double v_acc_demo_hatchback, double v_lead, double *d_safe) 
{
  ...
  {
    bool __assertCondition = v_acc_demo_hatchback > v_lead + 2;
    if ( !__assertCondition ) 
    {
      ___id->__preconditionFailed_samplePre = true;
      ___BlockMessages____block_messagelist_Contract_Failed("ACC_SafeDistance_Atomic_ssd_safe_distance_computation_samplePre", "BlockInterfaces:ssd_safe_distance_computationInterface_hatchback:1?r:f599b715-12fa-44c1-a288-a72fd0bffe50(_30_Architecture)#6504848816738695791");
      printf("v_lead=%f\n",v_lead);;
      printf("v_acc_demo_hatchback=%f\n",v_acc_demo_hatchback);;
  ...
  double coeff_g = 0.7 * 9.807;
  
  double D_BRAKE_sedan = ((v_acc_demo_hatchback * v_acc_demo_hatchback) / (2.0 * coeff_g));
  
  (*d_safe) = (v_acc_demo_hatchback * ___id->T_SAFE) + (fabs(v_lead - v_acc_demo_hatchback) * 0.2) + (D_BRAKE_sedan);
  assert((*d_safe) == 20.5);
  ...
  {
    bool __assertCondition = (*d_safe) >= COMMONUNITS_cm_to_m_double(150);
    if ( !__assertCondition ) 
    {
      ___BlockMessages____block_messagelist_Contract_Failed("ACC_SafeDistance_Atomic_ssd_safe_distance_computation_valid_d_safe", "BlockInterfaces:ssd_safe_distance_computationInterface_hatchback?r:f599b715-12fa-44c1-a288-a72fd0bffe50(_30_Architecture)#3980660603150638789");
      printf("d_safe=%f\n",(*d_safe));;
      
    }
  }
  {
    bool __assertCondition = !((v_acc_demo_hatchback > (v_lead + 2))) || (*d_safe) > 0;
    if ( !__assertCondition ) 
    {
      ___BlockMessages____block_messagelist_Contract_Failed("ACC_SafeDistance_Atomic_ssd_safe_distance_computation_temporalCondition", "BlockInterfaces:ssd_safe_distance_computationInterface_hatchback:2?r:f599b715-12fa-44c1-a288-a72fd0bffe50(_30_Architecture)#3980660603150649888");
      printf("v_acc_demo_hatchback=%f\n",v_acc_demo_hatchback);;
      printf("v_lead=%f\n",v_lead);;
      printf("d_safe=%f\n",(*d_safe));;
...
void  ACC_SafeDistance_Atomic____ssd_distance_error_execute(ACC_SafeDistance_Atomic____ssd_distance_error_data_t *___id, double d_lead, double d_safe_new, double *d_error) 
{
  ...
  {
    bool __assertCondition = (d_lead > 0.0) && (d_lead < 300.0);
    if ( !__assertCondition ) 
    {
...
void  ACC_SafeDistance_Atomic____ssd_update_pid_execute(ACC_SafeDistance_Atomic____ssd_update_pid_data_t *___id, double v_acc, double v_pid, double *v_set) 
{
  ...
  {
    bool __assertCondition = (v_acc >= 0.0) && (v_acc <= 50.0);
    if ( !__assertCondition ) 
    {
  ...
  {
    bool __assertCondition = (v_pid >= 0.0) && (v_pid <= 100.0);
    if ( !__assertCondition ) 
    {
  ...
  {
    bool __assertCondition = !((v_acc >= 0.0) && (v_acc <= 50.0) && (v_pid >= 0.0) && (v_pid <= 50.0)) || ((*v_set) >= 0.0) && ((*v_set) <= 50.0);
    if ( !__assertCondition ) 
    {
